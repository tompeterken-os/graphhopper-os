...
public class CarPriorityParser implements TagParser {
    ...
    public void handleWayTags(int edgeId, EdgeIntAccess edgeIntAccess, ReaderWay way, IntsRef relationFlags) {
        ...
        if (highwayValue == null) {
            if (FerrySpeedCalculator.isFerry(way))
                priorityWayEncoder.setDecimal(false, edgeId, edgeIntAccess, PriorityCode.getValue(handlePriority(way, priorityFromRelation, false)));
                if (priorityWayEncoder.isStoreTwoDirections())
                    priorityWayEncoder.setDecimal(true, edgeId, edgeIntAccess, PriorityCode.getValue(handlePriority(way, priorityFromRelation, true)));
        } else {
            priorityWayEncoder.setDecimal(false, edgeId, edgeIntAccess, PriorityCode.getValue(handlePriority(way, priorityFromRelation, false)));
            if (priorityWayEncoder.isStoreTwoDirections())
                priorityWayEncoder.setDecimal(true, edgeId, edgeIntAccess, PriorityCode.getValue(handlePriority(way, priorityFromRelation, true)));
    ...
    void collect(ReaderWay way, TreeMap<Double, Integer> weightToPrioMap, Boolean reverse) {
        ...
        if (way.hasTag("motor_vehicle", "private"))
            weightToPrioMap.put(40d, CUSTOM_DESTINATION.getValue());
